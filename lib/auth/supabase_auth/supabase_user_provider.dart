...
class ControleFinanceiroSupabaseUser extends BaseAuthUser {
  ControleFinanceiroSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> controleFinanceiroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ControleFinanceiroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
